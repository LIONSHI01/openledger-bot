...
async function generateTokenForAddress(address, agent, delay = 60000) {
  ...
  while (true) {
    try {
      ...
      console.error(
        `生成钱包 ${address} 的令牌时出错，尝试次数 ${attempt}:`,
        error.message
      );
...
async function getAccountID(token, address, index, useProxy, delay = 60000) {
  ...
  const agent =
    useProxy && proxyUrl ? new HttpsProxyAgent(proxyUrl) : undefined;
  ...
  while (true) {
    try {
      ...
      console.log(
        `\x1b[33m[${
          index + 1
        }]\x1b[0m 账户ID \x1b[36m${acctID}\x1b[0m, 代理: \x1b[36m${proxyText}\x1b[0m`
      );
      return;
    } catch (error) {
      console.error(
        `\x1b[33m[${
          index + 1
        }]\x1b[0m 获取钱包 ${address} 的账户ID时出错，尝试次数 ${attempt}:`,
        error.message
      );
...
async function getAccountDetails(
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      const rewardRealtimeResponse = await axios.get(
      ...
      const rewardHistoryResponse = await axios.get(
      ...
      const totalHeartbeats = parseInt(
        rewardRealtimeResponse.data.data[0]?.total_heartbeats || 0,
        10
      );
      const totalPointFromReward = parseFloat(
        rewardResponse.data.data?.totalPoint || 0
      );
      ...
      console.log(
        `\x1b[33m[${index + 1}]\x1b[0m 钱包 \x1b[36m${address}\x1b[0m, ` +
          ...
          `总积分 \x1b[32m${total.toFixed(
            2
          )}\x1b[0m (\x1b[33m${epochName}\x1b[0m), ` +
          `代理: \x1b[36m${proxyText}\x1b[0m`
      );
      return;
    } catch (error) {
      console.error(
        `获取账户详情时出错，钱包地址 ${address}，尝试次数 ${attempt}:`,
        error.message
      );
      if (attempt < retries) {
...
async function checkAndClaimReward(
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      if (!claimed) {
        const claimRewardResponse = await axios.get(
      ...
      console.error(
        `领取钱包 ${address} 的奖励时出错，尝试次数 ${attempt}:`,
        error.message
      );
      if (attempt < retries) {
        console.log(`${delay / 1000} 秒后重试...`);
...
function connectWebSocket({ token, workerID, id, address }, index, useProxy) {
  ...
  function sendHeartbeat() {
    ...
    console.log(
      `\x1b[33m[${index + 1}]\x1b[0m 账户ID \x1b[36m${
        accountIDs[address]
      }\x1b[0m: ` +
  ...
  ws.on("error", (err) => {
    console.error(
      `\x1b[33m[${index + 1}]\x1b[0m 账户ID \x1b[36m${
        accountIDs[address]
      }\x1b[0m: ` + `WebSocket错误，工作者ID \x1b[33m${workerID}\x1b[0m:`,
      err
    );
  });
