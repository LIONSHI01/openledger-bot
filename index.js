  ...
  while (true) {
    try {
      ...
      console.error(
        `\x1b[33m[${
          index + 1
        }]\x1b[0m 获取钱包 ${address} 的账户ID失败，第 ${attempt} 次尝试:`,
        error.message
      );
...
async function getAccountDetails(
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      console.error(
        `获取钱包 ${address} 账户详情失败，第 ${attempt} 次尝试:`,
        error.message
      );
      if (attempt < retries) {
...
async function checkAndClaimReward(
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      console.error(
        `领取钱包 ${address} 奖励失败，第 ${attempt} 次尝试:`,
        error.message
      );
      if (attempt < retries) {
        console.log(`${delay / 1000} 秒后重试...`);
...
function connectWebSocket({ token, workerID, id, address }, index, useProxy) {
  ...
  ws.on("error", (err) => {
    console.error(
      `\x1b[33m[${index + 1}]\x1b[0m 账户ID \x1b[36m${
        accountIDs[address]
      }\x1b[0m: ` + `工作ID \x1b[33m${workerID}\x1b[0m 的WebSocket错误:`,
      err
    );
  });
